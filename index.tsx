...
const genVideoClick = async (
  ...
  const videoWidth = aspectRatio === '9:16' ? VIDEO_HEIGHT : VIDEO_WIDTH;
  const videoHeight = aspectRatio === '9:16' ? VIDEO_WIDTH : VIDEO_HEIGHT;
...
const generateNewVideo = async (
  ...
  console.log('generateNewVideo', prompt, imgSrc, aspectRatio);
...
const assetUrls: TldrawProps['assetUrls'] = {
  icons: {
    ...
    save: await loadIcon('/save.svg'),
    history: await loadIcon('/history.svg'),
  },
};
...
const OverlayComponent = track(() => {
  ...
  return (
    <div
      ...
      <PromptBar
        onSubmit={async (prompt, image, action, aspectRatio) => {
          try {
            if (action === 'video') {
              await generateNewVideo(editor, prompt, image, aspectRatio);
            } else {
